...
const RecipeDetialScreen = () => {
  ...
  useEffect(() => {
    const checkIfSaved = async () => {
      try {
        ...
        const isRecipeSaved = favorites.some(
          (fav: Favorite) => fav.recipeId === parseInt(recipeId as string)
        );
    ...
    const loadRecipeDetail = async () => {
      ...
      try {
        ...
        if (mealData) {
          const transfromedRecipe = MealAPI.transformMealData(mealData);
  ...
  const getYoutubeEmberUrl = (url: string) => {
    // example url: https://www.youtube.com/watch?v=fLIl6jypzkI&t=12495s
  ...
  if (loading)
    return (
      <LoadingSpinner size={"large"} message="Loading recipe details..." />
    );
  return (
          ...
          <View style={recipeDetailStyles.floatingButtons}>
            ...
            <TouchableOpacity
              style={[
                ...
                { backgroundColor: isSaving ? COLORS.card : COLORS.primary },
              ]}
...
export default RecipeDetialScreen;
